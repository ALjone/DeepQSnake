...
cdef class SnakeGame:
    ...
    cdef (np.int32_t, np.int32_t) head_position
    # The positions of the snake's body
    cdef list body_positions
    # The position of the apple
    cdef (np.int32_t, np.int32_t) apple_position

    cdef int moves

    cdef int lifespan 
    
    cdef np.int32_t[:, :, :] game_map

    def __init__(self, rows: int, cols: int, lifespan: int):
        self.game_map = np.zeros((3, rows, cols), dtype=np.int32)
    ...
    cpdef tuple step(self, action: int):
        reward = 0
        cdef (np.int32_t, np.int32_t) new_head_position
        ...
        if new_head_position[0] < 0 or new_head_position[0] >= self.game_map.shape[1]:
            return np.asarray(self.game_map), -1, True  # Invalid position, return reward -1 and done = True
        if new_head_position[1] < 0 or new_head_position[1] >= self.game_map.shape[2]:
            return np.asarray(self.game_map), -1, True  # Invalid position, return reward -1 and done = True

        # Update game map with new head position
        self.game_map[0, self.head_position[0], self.head_position[1]] = 0
        self.game_map[0, new_head_position[0], new_head_position[1]] = 1
        ...
        if np.sum(self.game_map[1]) >= self.game_map.size**2-1:
    ...
    cpdef np.ndarray reset(self):
        self.game_map = np.zeros((3, self.game_map.shape[1], self.game_map.shape[2]), dtype=np.int32)
    ...
    cdef void update_state(self):
        ...
        #state = np.zeros((3, self.game_map.shape[0], self.game_map.shape[1]), dtype=np.int)
        self.game_map[:, :, :] = 0
        ...
        cdef (np.int32_t, np.int32_t) body_pos 
